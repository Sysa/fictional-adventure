...
"""
TO DO, improvements:
- write more integration tests (using pytest+fixtures)
- add some sort of sequence for transaction_cart, as in current implementation it took timestamp,
    because we are not using UUID/snowflakes here, and we will use something like last_trx_time = 00.00.00 or timestamp as ID,
    so in case when more than 1 transaction came at the exact same time - this case is not covered and will lead to collision.
- add receiver/router of the input data (complete Receiver class) to make startStream() look more great
- card_overdraft limit should be added to the account instance, but then it will be serialized as well
"""
